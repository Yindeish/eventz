...
const EventsSearch = () => {
    ...
    const { currentFeed } = useAppSelector((s: RootState) => s.search)


    return (
        <Tv className="w-full h-full bg-white pt-[24px] px-[20px]">

            <Urbanist className="text-gray-900 text-[15px] font-bold py-2">{currentFeed?.summary}</Urbanist>

            {Number(currentFeed?.events?.length) < 1 ? (
